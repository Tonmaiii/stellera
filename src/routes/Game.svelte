<script lang="ts">
	...
	let fov: number;
	const updateOverlay = (ctx: CanvasRenderingContext2D, _fov: number, ra: number) => {
	...
	const flashStar = (ctx: CanvasRenderingContext2D, star: star) => {
		if (!engine) return;
		const i = star.index;
		let x = engine.starScreenPos[3 * i];
		let y = -engine.starScreenPos[3 * i + 1];
		const w = engine.starScreenPos[3 * i + 2];
	...
	const drawStarLabel = (
		...
		const i = star.index;
		if (engine.starScreenPos[3 * i + 2] > 1) return;
		const x = ((engine.starScreenPos[3 * i] + 1) / 2) * canvas.width;
		const y = -((engine.starScreenPos[3 * i + 1] - 1) / 2) * canvas.height;
		const size = starSize(star.magnitude);
		...
		if (label.correct) {
			...
			if (star.display_name !== star.designation_name && star.designation_name) {
				ctx.fillText(star.designation_name, x, y + (size / 2) * (7 / 5) + 36);
			}
		} else {
			ctx.fillStyle = `rgba(255, 0, 0, ${alpha})`;

			if (useDesignation) {
				ctx.fillText(star.designation_name, x, y + (size / 2) * (7 / 5) + 16);
			} else {
				ctx.fillText(star.display_name, x, y + (size / 2) * (7 / 5) + 16);
			}
		}
	...
	const handleClick = (e: MouseEvent) => {
		...
		for (let i = 0; i < starsCount; i++) {
			...
			if (
				delta < closest &&
				(delta <= clickRange ** 2 || delta <= (starSize(star.magnitude) / 2) ** 2)
			) {
				closest = delta;
				closestStar = star;
			}
	...
	const update = (ctx: CanvasRenderingContext2D) => {
		if (!engine) return;
		engine.update();
		updateOverlay(ctx, engine.playerController.fov, engine.playerController.ra);
